...
def get_client(
  ...
  print('Connecting to gCloud IOT')
...
def read_sensors(device, ser, pipePressure, resTemp, resPH, resEC, sens5):
  ...
  try:
    ...
    if device == 'pi':
      s = {}
      response = serial_send_and_receive(ser, "1\n")
...
def serial_send_and_receive(ser, theinput):
  ...
  try:
    state = ser.readline().decode('utf-8').rstrip()
  except:
    state = "empty"
...
def main(argv):
  ...
  while True:
    ...
    sleep(10)  # Wait between readings
